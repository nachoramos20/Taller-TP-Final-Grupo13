...
void AttackCommand::execute(World& world) {
    ...
    world.push_message(target_id, 1, "Recibiste " + std::to_string(damage) + " de daño de " + std::string(attacker->username) + crit_str + ".");
...
void AttackNpcCommand::execute(World& world) {
    ...
    bool is_ranged = (w.kind == ItemKind::WEAPON_RANGED || w.kind == ItemKind::WEAPON_MAGIC);
    int dist = Equations::manhattan_distance(attacker->pos_x, attacker->pos_y, npc->pos_x, npc->pos_y);
    if (dist > (is_ranged ? w.range : 1)) {
    ...
    if (crit) {
        ...
        uint16_t npc_def = static_cast<uint16_t>(Equations::rand_range(tpl.defense_min, tpl.defense_max));
    ...
    if (npc->hp <= damage) {
        ...
        world.push_message(client_id, 1, "¡Mataste al " + tpl.name + "! +" + std::to_string(gold) + " oro.");
