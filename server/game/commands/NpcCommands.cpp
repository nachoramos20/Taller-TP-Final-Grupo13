...
NpcInteractCommand::NpcInteractCommand(uint16_t c, uint16_t n): client_id(c), npc_id(n) {}
...
void NpcInteractCommand::execute(World& world) {
    ...
    if (dist > 2) {
        world.push_message(client_id, 0, "Estás demasiado lejos de " + tpl.name + ". Acércate.");
        return;
    }
