...
void EquipCommand::execute(World& world) {
    ...
    if (!p || p->is_ghost)
        return;
    if (inv_slot >= PlayerData::INVENTORY_SIZE)
        return;
    ...
    if (def.kind == ItemKind::NONE || def.kind == ItemKind::POTION || def.kind == ItemKind::GOLD)
        return;
    ...
    switch (slot) {
        case EquipSlot::WEAPON:
            p->equipped_weapon = inv_slot;
            break;
        case EquipSlot::ARMOR:
            p->equipped_armor = inv_slot;
            break;
        case EquipSlot::HELMET:
            p->equipped_helmet = inv_slot;
            break;
        case EquipSlot::SHIELD:
            p->equipped_shield = inv_slot;
            break;
    }
}
...
void DropCommand::execute(World& world) {
    PlayerData* p = world.get_player_mutable(client_id);
    if (!p || inv_slot >= PlayerData::INVENTORY_SIZE)
        return;
    ...
    if (p->equipped_weapon == inv_slot)
        p->equipped_weapon = 0xFF;
    if (p->equipped_armor == inv_slot)
        p->equipped_armor = 0xFF;
    if (p->equipped_helmet == inv_slot)
        p->equipped_helmet = 0xFF;
    if (p->equipped_shield == inv_slot)
        p->equipped_shield = 0xFF;
...
void MoveItemCommand::execute(World& world) {
    ...
    if (!p)
        return;
    if (from_slot >= PlayerData::INVENTORY_SIZE || to_slot >= PlayerData::INVENTORY_SIZE)
        return;
    if (from_slot == to_slot)
        return;
    if (p->inventory[from_slot] == 0)
    ...
    auto remap = [&](uint8_t& equipped_slot) {
        if (equipped_slot == from_slot)
            equipped_slot = to_slot;
        else if (equipped_slot == to_slot)
            equipped_slot = from_slot;
    };
...
void UseItemCommand::execute(World& world) {
    ...
    if (item_id == ItemId::HEALTH_POTION) {
        ...
        world.push_message(client_id, 0,
                           "Usaste Pocion de Vida. HP +" + std::to_string(def.min_value));
    } else if (item_id == ItemId::MANA_POTION) {
        p->mp = std::min(p->max_mp, static_cast<uint16_t>(p->mp + def.min_value));
        world.push_message(client_id, 0,
                           "Usaste Pocion de Mana. MP +" + std::to_string(def.min_value));
    }
    p->inventory[inv_slot] = 0;
}
