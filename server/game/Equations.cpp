...
bool is_pvp_allowed(uint8_t attacker_lvl, uint8_t target_lvl) {
    ...
    if (attacker_lvl <= f.pvp_min_level || target_lvl <= f.pvp_min_level) return false;
    return std::abs(static_cast<int>(attacker_lvl) - static_cast<int>(target_lvl)) <= f.pvp_max_level_delta;
}
...
uint32_t exp_per_damage(uint16_t damage, uint8_t my_level, uint8_t other_level) {
    const auto& f = GameConfig::get().formulas();
    int factor = std::max(static_cast<int>(other_level) - static_cast<int>(my_level) + f.per_damage_base_factor, 0);
