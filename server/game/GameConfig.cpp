...
void GameConfig::load_game_config(const std::string& path) {
    ...
    // Leveling
    if (auto* lv = tbl["leveling"].as_table()) {
        _formulas.exp_base           = (*lv)["exp_base"].value_or<double>(1000.0);
        _formulas.exp_exponent       = (*lv)["exp_exponent"].value_or<double>(1.8);
        _formulas.pvp_min_level      = (*lv)["pvp_min_level"].value_or<int>(12);
        _formulas.pvp_max_level_delta= (*lv)["pvp_max_level_delta"].value_or<int>(10);
    }

    // Combat
    if (auto* cb = tbl["combat"].as_table()) {
        _formulas.crit_multiplier        = (*cb)["crit_multiplier"].value_or<double>(2.0);
        _formulas.crit_chance            = (*cb)["crit_chance"].value_or<double>(0.05);
        _formulas.dodge_cap              = (*cb)["dodge_cap"].value_or<double>(0.30);
        _formulas.dodge_per_agi          = (*cb)["dodge_per_agi"].value_or<double>(0.005);
        _formulas.attack_cooldown_melee  = (*cb)["attack_cooldown_melee"].value_or<int>(10);
        _formulas.attack_cooldown_spell  = (*cb)["attack_cooldown_spell"].value_or<int>(12);
    }

    // EXP formulas
    if (auto* ef = tbl["exp_formulas"].as_table()) {
        _formulas.per_damage_base_factor = (*ef)["per_damage_base_factor"].value_or<int>(10);
        _formulas.kill_exp_rand_max      = (*ef)["kill_exp_rand_max"].value_or<double>(0.1);
    }

    // NPC drops
    if (auto* nd = tbl["npc_drops"].as_table()) {
        _formulas.gold_rand_max       = (*nd)["gold_rand_max"].value_or<double>(0.2);
        _formulas.drop_chance_nothing = (*nd)["drop_chance_nothing"].value_or<double>(0.80);
        _formulas.drop_chance_gold    = (*nd)["drop_chance_gold"].value_or<double>(0.88);
        _formulas.drop_chance_potion  = (*nd)["drop_chance_potion"].value_or<double>(0.89);
        _formulas.drop_chance_item    = (*nd)["drop_chance_item"].value_or<double>(0.90);
        _formulas.gold_drop_min_frac  = (*nd)["gold_drop_min_frac"].value_or<double>(0.01);
        _formulas.gold_drop_max_frac  = (*nd)["gold_drop_max_frac"].value_or<double>(0.20);
    }

    // Meditation
    if (auto* med = tbl["meditation"].as_table()) {
        _formulas.mp_regen_per_tick = (*med)["mp_regen_per_tick"].value_or<int>(2);
    }

    // Resurrection
    if (auto* res = tbl["resurrection"].as_table()) {
        _formulas.respawn_x    = (*res)["respawn_x"].value_or<int>(40);
        _formulas.respawn_y    = (*res)["respawn_y"].value_or<int>(25);
        _formulas.hp_fraction  = (*res)["hp_fraction"].value_or<double>(0.25);
    }
}
