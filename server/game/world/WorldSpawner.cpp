...
bool WorldSpawner::pick_free_cell(const SpawnZone& z, uint16_t& out_x, uint16_t& out_y) {
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        if (!collision.in_bounds(cx, cy))
            continue;
        if (collision.is_occupied(cx, cy))
            continue;
        if (in_safe_zone(cx, cy))
            continue;  // no spawnear en zona segura
        out_x = cx;
        out_y = cy;
        return true;
    }
...
std::vector<WorldSpawner::PendingSpawn> WorldSpawner::tick(uint16_t total_alive) {
    ...
    for (uint8_t i = 0; i < zones.size(); ++i) {
        ...
        if (z.allowed_types.empty())
            continue;
        if (total_alive >= global_cap)
            break;
