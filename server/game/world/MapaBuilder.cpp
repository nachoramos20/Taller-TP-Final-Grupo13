...
MapaBuilder::MapaBuilder() { collision_map.resize(MAP_W * MAP_H, 0); }

TileDTO& MapaBuilder::get_tile(MapaDTO& mapa, uint16_t x, uint16_t y) {
    return mapa.tiles[y * mapa.width + x];
}

void MapaBuilder::fill_rect(MapaDTO& mapa, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                            uint16_t floor_id, uint16_t obj_id, uint16_t obj_sup_id) {
    for (int y = y1; y <= y2; y++) {
        for (int x = x1; x <= x2; x++) {
...
void MapaBuilder::place_object_sup(MapaDTO& mapa, uint16_t x, uint16_t y, uint16_t obj_sup_id) {
    get_tile(mapa, x, y).object_superior_id = obj_sup_id;
}

uint16_t MapaBuilder::pasto_random() { return F_PASTO_BASE + (rand() % 7); }

uint16_t MapaBuilder::arbol_bosque_random() {
    static const uint16_t arboles[] = {O_ARBOL_RECTO_1,   O_ARBOL_RECTO_2, O_ARBOL_RECTO_3,
                                       O_PINO_RECTO,      O_PINO_RECTO_2,  O_PINO_TORCIDO,
                                       O_PINO_TORCIDO_IZQ};
    return arboles[rand() % 7];
}

uint16_t MapaBuilder::flores_random() { return O_FLORES_BASE + (rand() % 7); }
...
void MapaBuilder::build_acantilados(MapaDTO& mapa) {
    ...
    // borde oeste
    for (int y = 6; y < 99; y += 6) get_tile(mapa, 0, y).floor_id = F_AC_OESTE;
}
...
void MapaBuilder::build_pasto(MapaDTO& mapa) {
    ...
    for (int y = ZJ_Y1; y <= 98; y += 2) get_tile(mapa, 53, y).floor_id = pasto_random();

    for (int y = 1; y <= ZJ_Y1 - 1; y++)
        for (int x = 0; x <= FRANJA_X1 - 1; x++) get_tile(mapa, x, y).floor_id = F_PASTO_BASE;

    for (int y = ZJ_Y2 + 1; y <= 98; y++)
        for (int x = 0; x <= FRANJA_X1 - 1; x++) get_tile(mapa, x, y).floor_id = F_PASTO_BASE;

    for (int y = 1; y <= ZJ_Y2; y++)
        for (int x = 0; x <= ZJ_X1 - 1; x++) get_tile(mapa, x, y).floor_id = F_PASTO_BASE;
...
void MapaBuilder::build_caminos(MapaDTO& mapa) {
    ...
    for (int y = CIU_Y2 + 1; y <= PUE_Y1 - 1; y++)
        for (int x = CAM_V_X1; x <= CAM_V_X2; x++) get_tile(mapa, x, y).floor_id = F_PASTO_1X1;
...
void MapaBuilder::build_ciudad(MapaDTO& mapa) {
    ...
    for (int x = CIU_X1; x <= CIU_X2; x++) get_tile(mapa, x, CIU_Y1 - 1).floor_id = F_FRANJA_CIU_N;
    for (int x = CIU_X1; x <= CIU_X2; x++) get_tile(mapa, x, CIU_Y2 + 1).floor_id = F_FRANJA_CIU_S;
    for (int y = CIU_Y1; y <= CIU_Y2; y++) get_tile(mapa, CIU_X2 + 1, y).floor_id = F_FRANJA_CIU_E;
    for (int y = CIU_Y1; y <= CIU_Y2; y++) get_tile(mapa, CIU_X1 - 1, y).floor_id = F_FRANJA_CIU_W;
    ...
    for (int y = CIU_Y1 - 2; y <= CIU_Y1; y++) get_tile(mapa, CIU_X1 - 2, y).floor_id = F_PASTO_1X1;
...
void MapaBuilder::build_pueblo(MapaDTO& mapa) {
    for (int y = PUE_Y1; y <= PUE_Y2; y++)
        for (int x = PUE_X1; x <= PUE_X2; x++) get_tile(mapa, x, y).floor_id = F_TIERRA_BASE;

    // Franja de transición pasto->pueblo
    for (int x = PUE_X1; x <= PUE_X2; x++) get_tile(mapa, x, PUE_Y1 - 1).floor_id = F_FRANJA_PUE_N;
    for (int x = PUE_X1; x <= PUE_X2; x++) get_tile(mapa, x, PUE_Y2 + 1).floor_id = F_FRANJA_PUE_S;
    for (int y = PUE_Y1; y <= PUE_Y2; y++) get_tile(mapa, PUE_X2 + 1, y).floor_id = F_FRANJA_PUE_E;
    for (int y = PUE_Y1; y <= PUE_Y2; y++) get_tile(mapa, PUE_X1 - 1, y).floor_id = F_FRANJA_PUE_W;
...
void MapaBuilder::build_cementerio(MapaDTO& mapa) {
    ...
    for (int x = CEM_X1; x <= CEM_X2; x++) get_tile(mapa, x, CEM_Y1 - 1).floor_id = F_FRANJA_PUE_N;
    for (int x = CEM_X1; x <= CEM_X2; x++) get_tile(mapa, x, CEM_Y2 + 1).floor_id = F_FRANJA_PUE_S;
    for (int y = CEM_Y1; y <= CEM_Y2; y++) get_tile(mapa, CEM_X2 + 1, y).floor_id = F_FRANJA_PUE_E;
    for (int y = CEM_Y1; y <= CEM_Y2; y++) get_tile(mapa, CEM_X1 - 1, y).floor_id = F_FRANJA_PUE_W;
...
void MapaBuilder::build_costa(MapaDTO& mapa) {
    ...
    // franja pasto->arena
    for (int y = 1; y <= 99; y += 2) get_tile(mapa, FRANJA_X1, y).floor_id = F_ARENA_FRANJA;

    // arena pura
    for (int y = 1; y <= 99; y++)
        for (int x = ARENA_X1; x <= OLAS_X2; x++) get_tile(mapa, x, y).floor_id = F_ARENA;

    // olas animadas
    for (int y = 1; y <= 99; y++)
        for (int x = OLAS_X1; x <= OLAS_X2; x += 2) place_object_sup(mapa, x, y, O_COSTA);

    // agua
    for (int y = 1; y <= 99; y++)
        for (int x = AGUA_X1; x <= AGUA_X2; x++) get_tile(mapa, x, y).floor_id = F_AGUA;
...
void MapaBuilder::build_mazmorra(MapaDTO& mapa) {
    ...
    mark_collision_rect(109, 30, 109,
                        80);  // pared izquierda la pared derecha ya queda bloqueada x limites del
                              // mapa despues hay q revisar si este es el comportamiento q esperamos
                              // o movemos la mazmorra 1 tile a la izq
}
...
void MapaBuilder::mark_collision_rect(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
    for (uint16_t y = y1; y <= y2; y++)
        for (uint16_t x = x1; x <= x2; x++) collision_map[get_index(x, y)] = 1;
}

std::vector<uint8_t> MapaBuilder::take_collision() { return std::move(collision_map); }
