...
ItemKind GameConfig::parse_item_kind(const std::string& s) {
    if (s == "weapon_melee")
        return ItemKind::WEAPON_MELEE;
    if (s == "weapon_ranged")
        return ItemKind::WEAPON_RANGED;
    if (s == "weapon_magic")
        return ItemKind::WEAPON_MAGIC;
    if (s == "armor")
...
bool GameConfig::item_exists(ItemId id) const { return _items.count(static_cast<uint8_t>(id)) > 0; }
...
EquipSlot GameConfig::equip_slot_for(ItemKind kind) const {
    switch (kind) {
        ...
        default:
            throw std::runtime_error("Item no equipable");
    }
}
...
const std::vector<NpcId>& GameConfig::all_npc_ids() const { return _all_npc_ids; }

const SpellConfig& GameConfig::spell(uint8_t spell_id) const {
    auto it = _spells.find(spell_id);
    if (it == _spells.end())
        return _fallback_spell;
    return it->second;
}

const CombatFormulas& GameConfig::formulas() const { return _formulas; }
