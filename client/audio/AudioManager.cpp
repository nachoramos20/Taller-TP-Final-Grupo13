...
static constexpr int LOOPING_CHANNEL =
        2;  // reservado: loop gateado por condición (pasos largos, etc.)
static constexpr int SECONDARY_AMBIENT_CHANNEL =
        3;  // reservado: segundo ambiente en loop (viento del cementerio, etc.)
static constexpr int MEDITATION_CHANNEL_A = 4;  // reservado: loop de meditación (vueltas pares)
static constexpr int MEDITATION_CHANNEL_B =
        5;  // reservado: loop de meditación (vueltas impares, se solapan con A)
...
AudioManager::~AudioManager() {
    for (auto& [path, chunk]: _chunk_cache) Mix_FreeChunk(chunk);
    for (auto& [path, chunk]: _speech_chunk_cache) Mix_FreeChunk(chunk);
    for (auto& [path, chunk]: _ambient_chunk_cache) Mix_FreeChunk(chunk);
    if (_music)
...
void AudioManager::play_music_loop(const std::string& path) {
    ...
    if (!loaded)
        throw std::runtime_error(std::string("Mix_LoadMUS: ") + Mix_GetError());
...
Mix_Chunk* AudioManager::trim_silence(Mix_Chunk* chunk, uint32_t lead_pad_ms,
                                      uint32_t tail_pad_ms) const {
    if (!chunk || chunk->alen < 4)
        return chunk;
    ...
    if (format != AUDIO_S16LSB && format != AUDIO_S16MSB)
        return chunk;  // solo soporta PCM de 16 bits
    ...
    for (uint32_t f = 0; f < total_frames; f += window_frames) {
        ...
        if (rms > SILENCE_THRESHOLD) {
            if (first_loud == UINT32_MAX)
                first_loud = f;
            last_loud = end_f;
        }
    }
    if (first_loud == UINT32_MAX)
        return chunk;  // todo silencio: no tocar
...
Mix_Chunk* AudioManager::load_ambient_chunk(const std::string& path) {
    ...
    if (it != _ambient_chunk_cache.end())
        return it->second;
...
void AudioManager::play_effect_now(const std::string& path, float dist_tiles, float volume_scale) {
    ...
    int volume = static_cast<int>(
            (MAX_EFFECT_VOLUME - t * (MAX_EFFECT_VOLUME - MIN_EFFECT_VOLUME)) * volume_scale);
...
uint32_t AudioManager::chunk_duration_ms(Mix_Chunk* chunk) const {
    if (!chunk)
        return 600;  // estimacion si el asset no cargo
...
void AudioManager::speak_random(const std::vector<std::string>& paths, float dist_tiles,
                                uint32_t gap_ms) {
    if (paths.empty())
        return;
    const std::string& chosen = paths[static_cast<size_t>(rand()) % paths.size()];
...
void AudioManager::set_ambient_loop_on(int channel, std::string& tracked_path,
                                       const std::string& path, float dist_tiles,
                                       float max_audible_tiles) {
    if (dist_tiles > max_audible_tiles) {
...
void AudioManager::set_secondary_ambient_loop(const std::string& path, float dist_tiles,
                                              float max_audible_tiles) {
    float range = (max_audible_tiles > 0.0f) ? max_audible_tiles : MAX_AUDIBLE_TILES;
    set_ambient_loop_on(SECONDARY_AMBIENT_CHANNEL, _secondary_ambient_path, path, dist_tiles,
                        range);
}

void AudioManager::set_looping_while_on(int channel, std::string& tracked_path,
                                        const std::string& path, bool active, float volume_scale) {
    if (!active) {
        if (!tracked_path.empty()) {
            Mix_HaltChannel(channel);
            tracked_path.clear();
...
void AudioManager::set_meditation_loop(const std::string& path, bool active, float volume_scale) {
    ...
    if (_meditation_active_channel == -1 || _meditation_path != path) {
        ...
        Mix_HaltChannel(MEDITATION_CHANNEL_A);
        Mix_HaltChannel(MEDITATION_CHANNEL_B);
        Mix_PlayChannel(MEDITATION_CHANNEL_A, chunk,
                        0);  // sin loop propio: update() encadena la siguiente vuelta
        ...
        _meditation_next_switch_ms =
                SDL_GetTicks() + (dur > MEDITATION_OVERLAP_MS ? dur - MEDITATION_OVERLAP_MS : 0);
    }
}
    ...
    if (_meditation_active_channel != -1 && now >= _meditation_next_switch_ms) {
        ...
        if (chunk) {
            int other = (_meditation_active_channel == MEDITATION_CHANNEL_A) ?
                                MEDITATION_CHANNEL_B :
                                MEDITATION_CHANNEL_A;
            ...
            uint32_t dur = chunk_duration_ms(chunk);
            _meditation_next_switch_ms =
                    now + (dur > MEDITATION_OVERLAP_MS ? dur - MEDITATION_OVERLAP_MS : 0);
        }
    }
}
