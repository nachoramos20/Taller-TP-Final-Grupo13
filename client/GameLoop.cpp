...
GameLoop::GameLoop(SDL2pp::Window& window, SDL2pp::Renderer& renderer,
                   ...
                   Queue<MapaDTO>* map_queue, std::atomic<bool>* connected, AudioManager* audio,
...
GameLoop::GameLoop(SDL2pp::Window& window, SDL2pp::Renderer& renderer,
                   Queue<Command>* command_queue, Queue<SnapshotDTO>* snapshot_queue,
                   Queue<MapaDTO>* map_queue, std::atomic<bool>* connected, AudioManager* audio,
                   const std::string& welcome_message, const std::string& username):
        _window(window),
        _renderer(renderer),
        ...
        _input(_camera, _player, _command_queue, _hud.chat(), _hud.stats(), _hud.inventory(),
               _hud.position_label()),
    ...
    _input.on_world_click(
            [this](int tile_x, int tile_y) { _actions.handle_world_click(tile_x, tile_y); });

    _input.on_use_potion([this]() { handle_use_potion(); });

    _input.on_resurrect([this]() { _actions.try_play_resurrect_sound(); });
...
void GameLoop::update(float dt) {
    ...
    if (_audio) {
        ...
        _audio_service.update_cemetery_ambient(
                distance_to_cemetery_zone(_player.tile_x, _player.tile_y));
    }
}
