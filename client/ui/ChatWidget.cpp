...
bool ChatWidget::handle_event(const SDL_Event& e) {
    if (e.type == SDL_KEYDOWN && e.key.keysym.sym == SDLK_RETURN) {
        if (!_input_active) {
            ...
            if (!_input_buffer.empty() && _on_submit)
                _on_submit(_input_buffer);
            _input_buffer.clear();
            _input_active = false;
            SDL_StopTextInput();
            return true;
...
void ChatWidget::render(int screen_w, int screen_h) {
    ...
    if (!_input_active) {
        while (!_messages.empty() && now - _messages.front().born_ms > MSG_LIFETIME_MS) {
            _messages.pop_front();
        }
    }
    ...
    if (!_input_active) {
        int hint_y = has_content ? y + std::min(total_lines, max_lines) * line_h + CHAT_PAD + 4 : y;
