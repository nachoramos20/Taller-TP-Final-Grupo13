...
void StatsPanel::render(int screen_w, int screen_h) {
    ...
    input.hp = _hp; input.max_hp = _max_hp;
    input.mp = _mp; input.max_mp = _max_mp;
    input.gold = _gold; input.exp = _exp;
    input.level = _level;
    input.meditating = _meditating; input.is_ghost = _is_ghost;
    input.cls = _cls; input.eq_weapon_item = _eq_weapon_item;
