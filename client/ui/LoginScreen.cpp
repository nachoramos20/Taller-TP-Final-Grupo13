...
LoginScreen::~LoginScreen() {
    SDL_StopTextInput();
    if (_font_lg)
        TTF_CloseFont(_font_lg);
    if (_font_md)
        TTF_CloseFont(_font_md);
    if (_font_sm)
        TTF_CloseFont(_font_sm);
    for (auto* t: _race_tex)
        if (t)
            SDL_DestroyTexture(t);
    if (_logo_tex)
        SDL_DestroyTexture(_logo_tex);
}
...
LoginResult LoginScreen::run() {
    ...
    res.do_register = (_screen == Screen::REGISTER_FORM || _screen == Screen::REGISTER_CLASS ||
...
bool LoginScreen::clicked(int mx, int my, const SDL_Rect& r) {
    ...
    if (_audio) {
        ...
        if (!click.empty())
            _audio->play_effect_at(click.front(), 0.0f);
    }
    return true;
}
...
void LoginScreen::handle_mouse_click(int mx, int my) {
    ...
    switch (_screen) {
        case Screen::SPLASH:
            if (clicked(mx, my, _btn_comenzar)) {
                _screen = Screen::MAIN;
            }
            break;
...
void LoginScreen::draw_text(const std::string& t, int x, int y, SDL_Color c, TTF_Font* f) {
    if (t.empty())
        return;
...
void LoginScreen::draw_text_centered(const std::string& t, int cx, int cy, SDL_Color c,
                                     TTF_Font* f) {
    TTF_Font* font = f ? f : _font_md;
    if (!font || t.empty())
        return;
...
void LoginScreen::draw_button(const SDL_Rect& r, const std::string& label, bool hovered,
                              bool selected) {
    SDL_SetRenderDrawBlendMode(_renderer.Get(), SDL_BLENDMODE_BLEND);
...
void LoginScreen::draw_input(const SDL_Rect& r, const std::string& text, bool active,
...
void LoginScreen::draw_race_card(int idx, int x, int y, int w, int h) {
    ...
    if (_race_tex[idx]) {
        ...
        if (img_w > img_h * aspect)
            img_w = static_cast<int>(img_h * aspect);
        else
            img_h = static_cast<int>(img_w / aspect);
        ...
        SDL_Color race_colors[4] = {{180, 140, 100, 255},
                                    {120, 180, 130, 255},
                                    {160, 120, 80, 255},
                                    {140, 120, 160, 255}};
    ...
    draw_text_centered(race_info(static_cast<uint8_t>(idx)).name, x + w / 2, y + h - 32, name_c,
                       _font_md);
...
void LoginScreen::render() {
    ...
    switch (_screen) {
        ...
        case Screen::LOGIN_FORM:
            ...
            break;
        case Screen::REGISTER_RACE:
            render_register_race();
            break;
        case Screen::REGISTER_CLASS:
            render_register_class();
            break;
        case Screen::REGISTER_FORM:
    ...
    if (!_error_msg.empty()) {
        int w = _window.GetWidth(), h = _window.GetHeight();
        draw_text_centered(_error_msg, w / 2, h - 30, SDL_Color{220, 80, 80, 255}, _font_md);
    }
...
void LoginScreen::render_splash() {
    ...
    if (!_logo_tex)
        _logo_tex = load_texture("assets/sprites/presentation/argentum.png");
    ...
    draw_text_centered("ESC para salir", sw / 2, sh - 25, UI_LOGIN_PLACEHOLDER_COLOR, _font_sm);
}
...
void LoginScreen::render_main() {
    ...
    draw_button(_btn_login, "Iniciar sesion", in_rect(_hover_x, _hover_y, _btn_login));
    draw_button(_btn_register, "Registrarse", in_rect(_hover_x, _hover_y, _btn_register));
...
void LoginScreen::render_register_form() {
    ...
    draw_text("(max 20 caracteres, sin espacios)", px + 16, py + 108, SDL_Color{80, 75, 60, 255},
              _font_sm);
