...
void StatsPanelRenderer::draw_text_right(const std::string& text, int right_x, int y, SDL_Color color) {
    if (text.empty() || !_font) return;
...
void StatsPanelRenderer::draw_bar(int x, int y, int w, int h,
...
StatsLayout StatsPanelRenderer::render(int screen_w, int screen_h, const StatsRenderInput& input) {
    ...
    uint32_t exp_current_level_floor = (input.level <= 1) ? 0
        : static_cast<uint32_t>(1000.0 * std::pow(static_cast<double>(input.level - 1), 1.8));

    uint32_t exp_next_level_ceil = (input.level >= 50) ? 0xFFFFFFFF
    ...
    float exp_frac = (input.level >= 50) ? 1.0f : static_cast<float>(exp_progress_in_level) / exp_needed_for_this_level;
