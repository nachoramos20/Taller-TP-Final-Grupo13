...
InventoryRenderer::InventoryRenderer(SDL2pp::Renderer& renderer, TTF_Font* font, TTF_Font* font_sm, int font_size)
    : _renderer(renderer), _font(font), _font_sm(font_sm), _font_size(font_size) {}

void InventoryRenderer::draw_text(const std::string& text, int x, int y,
...
void InventoryRenderer::draw_text_centered(const std::string& text, int cx, int cy,
                                           SDL_Color color, TTF_Font* font) {
    if (text.empty()) return;
    TTF_Font* f = font ? font : _font;
    if (!f) return;
...
void InventoryRenderer::draw_slot(const InventoryRenderInput& input, int x, int y, int size, int idx,
...
void InventoryRenderer::draw_equip_row(const InventoryRenderInput& input, int x, int y, int w, int h,
                                       const char* label, uint8_t slot_idx) {
    SDL_SetRenderDrawBlendMode(_renderer.Get(), SDL_BLENDMODE_BLEND);

    bool has_item = (slot_idx != 0xFF && slot_idx < InventoryRenderInput::INV_SIZE && (*input.inventory)[slot_idx] != 0);
...
InventoryLayout InventoryRenderer::render(int screen_w, int screen_h, const InventoryRenderInput& input) {
    ...
    for (const EqEntry& entry : eq_entries) {
        draw_equip_row(input, panel_x + padding, cy, grid_width, equip_row_height, entry.label, entry.slot_idx);
    ...
    if (input.selected_slot >= 0 && input.selected_slot < InventoryLayout::INV_SIZE
        ...
        bool is_equipped = (input.selected_slot == input.eq_wpn  || input.selected_slot == input.eq_arm ||
        ...
        if (is_equipped) {
            ...
            draw_text_centered(btn_label,
                               layout.equip_btn.x + button_width / 2, layout.equip_btn.y + button_height / 2,
        ...
        draw_text_centered("Tirar",
                           layout.drop_btn.x + button_width / 2, layout.drop_btn.y + button_height / 2,
