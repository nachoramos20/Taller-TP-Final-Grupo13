...
void SnapshotProcessor::apply_snapshot(WorldState& state, PlayerState& player,
                                       const SnapshotDTO& snap) {
...
void SnapshotProcessor::detect_deaths(WorldState& state, const PlayerState& player,
                                      const SnapshotDTO& snap) {
    for (const EntityDTO& prev: state.entities) {
        if (prev.entity_type != static_cast<uint8_t>(EntityType::NPC))
            continue;

        bool found = false;
        for (const EntityDTO& next_entity: *snap.entities)
            if (next_entity.entity_id == prev.entity_id) {
                found = true;
                break;
            }
        if (found)
    ...
    for (const EntityDTO& next_entity: *snap.entities) {
        if (next_entity.entity_type != static_cast<uint8_t>(EntityType::PLAYER))
            continue;
        if (next_entity.entity_id == state.my_entity_id || next_entity.is_ghost == 0)
            continue;

        for (const EntityDTO& prev: state.entities) {
            if (prev.entity_id == next_entity.entity_id && prev.is_ghost == 0) {
                _audio.player_death(
                        player.dist_to_player_tiles(next_entity.pos_x, next_entity.pos_y));
                break;
            }
        }
    }
...
void SnapshotProcessor::apply_spell_events(WorldState& state, const SnapshotDTO& snap) {
    if (!snap.spell_events)
        return;

    for (const SpellEventDTO& event: *snap.spell_events) {
        if (event.caster_id == state.my_entity_id)
            continue;  // ya lo spawneamos localmente
        ...
        for (const EntityDTO& entity: state.entities)
            if (entity.entity_id == event.caster_id) {
                caster = &entity;
                break;
            }
        if (!caster)
            continue;  // caster fuera de rango/no visible: no hay "desde dónde" dibujarlo

        state.spawn_projectile(caster->pos_x, caster->pos_y, event.target_x, event.target_y,
                               event.is_magic_projectile);
    }
}

void SnapshotProcessor::update_service_npc_range(int32_t& npc_id, float range_tiles,
                                                 const WorldState& state, const PlayerState& player,
    ...
    bool left =
            (npc == nullptr) || (player.dist_to_player_tiles(npc->pos_x, npc->pos_y) > range_tiles);
    if (!left)
        return;
...
void SnapshotProcessor::update_service_npc_ranges(WorldState& state, const PlayerState& player) {
    ...
    update_service_npc_range(state.bank_npc_id, interaction.bank_range_tiles, state, player,
                             nullptr);
...
void SnapshotProcessor::announce_chat_messages(const SnapshotDTO& snap) {
    ...
    for (const ChatMessageDTO& message: *snap.messages) {
        if (_chat)
            _chat->add_message(message.text);
...
void SnapshotProcessor::update_progression_sounds(WorldState& state, const SnapshotDTO& snap) {
    ...
    if (state.level_initialized && snap.level > state.last_level)
        _audio.level_up();
...
void SnapshotProcessor::update_panels(const SnapshotDTO& snap) {
    if (_stats) {
        ...
        _stats->update(snap.hp, snap.max_hp, snap.mp, snap.max_mp, snap.gold, snap.level, snap.exp,
                       snap.meditating != 0, snap.is_ghost != 0, snap.character_class,
...
void SnapshotProcessor::sync_own_equipment(WorldState& state, const SnapshotDTO& snap) {
    for (int i = 0; i < SnapshotDTO::INVENTORY_SIZE; i++) state.inventory[i] = snap.inventory[i];
...
void SnapshotProcessor::sync_own_position(const WorldState& state, PlayerState& player,
                                          const SnapshotDTO& snap) {
    if (!snap.entities)
        return;
    for (const EntityDTO& entity: *snap.entities) {
        if (entity.entity_id != snap.self_entity_id)
            continue;
