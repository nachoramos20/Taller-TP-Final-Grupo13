...
std::array<int, 4> parse_int4(toml::node_view<toml::node> node) {
    ...
    for (auto& elem : *arr) {
        if (i >= 4) break;
        if (auto iv = elem.as_integer()) out[i] = static_cast<int>(iv->get());
        i++;
    }
    return out;
}
    ...
    try {
        ...
        if (auto* items_tbl = config["item"].as_table()) {
            for (auto& [key, val] : *items_tbl) {
                ...
                entry.attack_sound_category = (*entry_tbl)["attack_sound_category"].value_or(std::string(""));
        ...
        if (auto* floor_tbl = config["floor_item_variants"].as_table()) {
            for (auto& [key, val] : *floor_tbl) {
                ...
                for (auto& elem : *arr) {
                    if (auto p = elem.value<std::string>()) variants.push_back(*p);
                }
...
const std::vector<std::string>& ItemVisualConfig::get_floor_variants(uint16_t floor_sprite_id) const {
