...
AnimationSystem::AnimationSystem(): _last_sprite_id(-1), _head_overlaps{38, 40, 38, 38} {}
    ...
    for (int dir = 0; dir < 4; dir++) {
        ...
        for (int f = 0; f < BodyLayout::DIR_N[dir]; f++) {
            _body_anims[dir].frames.push_back(
                    SDL2pp::Rect(f * BodyLayout::FRAME_W, BodyLayout::DIR_Y[dir],
                                 BodyLayout::FRAME_W, BodyLayout::DIR_H[dir]));
        }
    }
}
...
static int weapon_row(int dir_idx, const std::string& weapon_path) {
    if (!needs_north_south_swap(weapon_path))
        return dir_idx;
    if (dir_idx == AnimationSystem::DIR_SOUTH)
        return AnimationSystem::DIR_NORTH;
    if (dir_idx == AnimationSystem::DIR_NORTH)
        return AnimationSystem::DIR_SOUTH;
    return dir_idx;
}

void AnimationSystem::render_weapon(SDL2pp::Renderer& renderer, AssetManager& assets,
                                    const std::string& weapon_path, int dir_idx, int frame,
                                    const SDL2pp::Rect& body_dst, float scale) {
    if (weapon_path.empty())
        return;
    ...
    int overlap = static_cast<int>(
            15 * scale);  // se mete en el cuerpo, así parece que la tiene en la mano
    int wx, wy;
...
SpriteBounds AnimationSystem::render(SDL2pp::Renderer& renderer, AssetManager& assets,
                                     const std::string& body_path, const std::string& head_path,
                                     uint8_t sprite_id, Direction dir, int screen_x, int screen_y,
                                     uint32_t tick, bool is_moving, const EquipVisual* equip,
                                     bool is_ghost) {
    ...
    SDL2pp::Rect body_dst(screen_x - body_w / 2 + ClientConfig::instance().tile_size() / 2,
                          screen_y - body_h + ClientConfig::instance().tile_size(), body_w, body_h);
    int head_w = static_cast<int>(head_src.w * scale);
    int head_h = static_cast<int>(head_src.h * scale);
    SDL2pp::Rect head_dst(body_dst.x + (body_dst.w - head_w) / 2 +
                                  static_cast<int>(_head_offset_x[dir_idx] * scale),
    ...
    auto draw_shield = [&]() {
        ...
        int sx = weapon_on_left(dir_idx) ? body_dst.x + body_dst.w - overlap :
    ...
    auto draw_weapon = [&]() {
        if (!equip)
            return;
        if (is_ghost && !equip->weapon_path.empty())
            get_tex(equip->weapon_path);
    ...
    if (equip && !equip->armor_path.empty()) {
        ...
        SDL2pp::Rect armor_dst(body_dst.x, body_dst.y + body_dst.h - armor_h, body_dst.w, armor_h);
    ...
    if (equip && !equip->helmet_path.empty() && equip->helmet_src_w > 0) {
        ...
        SDL2pp::Rect helm_dst(
                head_dst.x + (head_dst.w - helm_w) / 2 +
                        static_cast<int>(equip->helmet_offset_x[dir_idx] * scale),
                head_dst.y + static_cast<int>(equip->helmet_offset_y[dir_idx] * scale), helm_w,
                helm_h);
    ...
    for (SDL2pp::Texture* tex: ghost_textures) tex->SetAlphaMod(255);

    return {head_dst.y, body_dst.x + body_dst.w / 2, body_dst.w};
}

SpriteBounds AnimationSystem::render_npc(SDL2pp::Renderer& renderer, AssetManager& assets,
                                         const std::string& sheet_path, int cols, int rows,
                                         int frame_w, int frame_h, Direction dir, int screen_x,
                                         int screen_y, uint32_t tick, bool is_moving, float scale,
                                         int draw_offset_y) {
    ...
    SDL2pp::Rect dst(screen_x + (ClientConfig::instance().tile_size() - dst_w) / 2,
                     top_y + draw_offset_y, dst_w, dst_h);
