...
void EntityRenderer::render_floor_item(const EntityDTO& e, int screen_x, int screen_y) {
    if (e.sprite_id == static_cast<uint8_t>(ItemId::BLOOD_STAIN))
        return;
    ...
    SDL2pp::Rect dst(screen_x, screen_y, ClientConfig::instance().tile_size(),
...
void EntityRenderer::render_npc(const EntityDTO& e, const WorldState& state, int screen_x,
                                int screen_y, Direction dir, bool moving) {
    const auto& npc_entry = NpcVisualConfig::instance().get(e.sprite_id);
    if (npc_entry.variants.empty())
        return;
    ...
    SpriteBounds bounds =
            _anim.render_npc(_renderer, _assets, variant.sheet_path, variant.cols, variant.rows,
                             variant.frame_w, variant.frame_h, dir, screen_x, screen_y,
...
EquipVisual EntityRenderer::build_equip_visual(uint8_t item_weapon, uint8_t item_armor,
                                               uint8_t item_helmet, uint8_t item_shield,
                                               uint8_t race_sprite_id) const {
    EquipVisual equip{};
    auto& cfg = ItemVisualConfig::instance();

    if (item_weapon != 0)
        equip.weapon_path = cfg.get(item_weapon).equip_path;
    if (item_armor != 0)
        equip.armor_path = cfg.get(item_armor).equip_path;
    if (item_shield != 0)
        equip.shield_path = cfg.get(item_shield).equip_path;
...
void EntityRenderer::render_player_like(const EntityDTO& e, const WorldState& state, int screen_x,
                                        int screen_y, Direction dir, bool moving) {
    ...
    EquipVisual equip =
            build_equip_visual(item_weapon, item_armor, item_helmet, item_shield, e.sprite_id);

    const SpriteEntry& sprite = _sprite_config.get(e.sprite_id);
    SpriteBounds bounds =
            _anim.render(_renderer, _assets, sprite.body_path, sprite.head_path, e.sprite_id, dir,
                         screen_x, screen_y, state.current_tick, moving, &equip, e.is_ghost != 0);
...
void EntityRenderer::render_entity_healthbar(const EntityDTO& entity, const SpriteBounds& bounds,
    ...
    if (entity.entity_type == static_cast<uint8_t>(EntityType::ITEM_FLOOR))
        return;
    ...
    const bool show_name =
            ((entity.entity_type == static_cast<uint8_t>(EntityType::PLAYER)) || is_service_npc) &&
    ...
    if (tex) {
        int name_y =
                is_service_npc ? bounds.top_y + name_label_offset_y : text_anchor_y - surf->h - 2;
