...
bool is_pvp_allowed_client(uint8_t attacker_level, uint8_t target_level) {
    ...
    if (attacker_level <= PVP_MIN_LEVEL || target_level <= PVP_MIN_LEVEL) return false;
    return std::abs(static_cast<int>(attacker_level) - static_cast<int>(target_level)) <= PVP_MAX_LEVEL_DELTA;
}
    ...
    for (const auto& e : _state.entities) {
        ...
        if (is_service_npc) {
            ...
            if (target_is_player && !is_pvp_allowed_client(_stats->current_level(), e.level)) {
                if (_chat) _chat->add_message("No puedes atacar a ese jugador (fair-play).");
                return;
            }
...
void PlayerActionController::handle_chat_command(const std::string& text) {
    auto dist_to_npc = [this](int32_t npc_id) -> float {
        for (const auto& e : _state.entities)
            if (static_cast<int32_t>(e.entity_id) == npc_id) return _player.dist_to_player_tiles(e.pos_x, e.pos_y);
        return 0.0f;
    };
...
void PlayerActionController::try_play_resurrect_sound() {
    for (const auto& e : _state.entities) {
        ...
        if (_player.manhattan_dist_to_player_tiles(e.pos_x, e.pos_y) > 2) continue;
