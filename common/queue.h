...
public:
    ...
    explicit Queue(unsigned int max_size): max_size(max_size), closed(false) {}
...
public:
    explicit Queue(unsigned int max_size): Queue<void*>(max_size) {}
    ...
    void push(T* const& val) { Queue<void*>::push(val); }

    // cppcheck-suppress duplInheritedMember
    T* pop() { return static_cast<T*>(Queue<void*>::pop()); }

    // cppcheck-suppress duplInheritedMember
    void close() { Queue<void*>::close(); }
