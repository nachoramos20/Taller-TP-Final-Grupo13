...
inline bool weapon_is_magic(uint8_t item_id) { return weapon_enables_spells(item_id); }
...
inline bool weapon_is_ranged(uint8_t item_id) {
    return item_id == static_cast<uint8_t>(ItemId::SIMPLE_BOW) ||
           item_id == static_cast<uint8_t>(ItemId::COMPOUND_BOW) || weapon_is_magic(item_id);
}
